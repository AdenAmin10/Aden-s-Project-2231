...
Branch created for this assignment: `component-abstract-class` (from `main`).

Design updates from Parts 1-3 to this Part 4 implementation:

1. Added `ScoreTrackerSecondary` to implement all enhanced methods declared in
   `ScoreTracker`.
2. Implemented convenience scoring methods by delegating to kernel
   `addPoints(team, points)` with valid contract values 1, 2, and 3.
3. Added contract-aware `leader()` logic with a precondition check
   (`assert !this.isTie()`), then selected the leading team by score comparison.
4. Implemented `resetGame()` by calling `clear()` from `Standard`, preserving
   layering and avoiding representation access.
5. Implemented `toString()`, `equals(Object)`, and `hashCode()` using only
   kernel observers (`score`, `fouls`, `period`) so behavior is representation
   independent.
