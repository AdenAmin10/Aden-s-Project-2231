...
Design updates from the proof-of-concept and brainstorming stage:

1. Simplified `Period` from an enum-style idea to an integer (`period()`), which
   avoids hard-coding overtime labels in the interface.
2. Replaced `getScore(...)`, `getFouls(...)`, and `getPeriod()` with compact
   kernel names `score(...)`, `fouls(...)`, and `period()`.
3. Replaced nullable `getLeader()` from brainstorming with `leader()` plus
   `@requires not this.isTie()` to remove null from the contract.
4. Deferred possession-related behavior to a future iteration to keep the
   kernel minimal and aligned with layering goals for Parts 3 and 4.
...
Created branch: `component-interfaces` (from `main`) for this assignment.
